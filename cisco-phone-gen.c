...
void show_text_input(Field *f) {
  ...
  while (1) {
    ...
    if (ch == 10 || ch == 13 || ch == KEY_ENTER) {
      ...
      if (pos > 0) {
        memmove(&f->value[pos - 1], &f->value[pos], len - pos + 1);
        pos--;
        len--;
      }
    } else if (ch == KEY_DC) { // Delete key
      if (pos < len) {
        memmove(&f->value[pos], &f->value[pos + 1], len - pos);
        len--;
      }
    } else if (ch == KEY_LEFT) {
      if (pos > 0) pos--;
      ...
      pos = len;
    } else if (ch >= 32 && ch <= 126 && len < 127 && pos < 58) {
      memmove(&f->value[pos + 1], &f->value[pos], len - pos + 1);
      f->value[pos] = ch;
      pos++;
      len++;
...
void save_xml() {
  ...
  // SIP Profile Wrapper
  fprintf(fp, "  <sipProfile>\n");
  fprintf(fp, "    <sipProxies>\n");
  fprintf(fp, "      <backupProxy>%s</backupProxy>\n", val("processNodeName1"));
  fprintf(fp, "      <backupProxyPort>%s</backupProxyPort>\n", port);
  fprintf(fp, "      <emergencyProxy>%s</emergencyProxy>\n", val("processNodeName1"));
  fprintf(fp, "      <emergencyProxyPort>%s</emergencyProxyPort>\n", port);
  fprintf(fp, "      <outboundProxy>%s</outboundProxy>\n", val("processNodeName1"));
  fprintf(fp, "      <outboundProxyPort>%s</outboundProxyPort>\n", port);
  fprintf(fp, "      <registerWithProxy>true</registerWithProxy>\n");
  fprintf(fp, "    </sipProxies>\n");

  fprintf(fp, "    <sipCallFeatures>\n");
  fprintf(fp, "      <cnfJoinEnabled>true</cnfJoinEnabled>\n");
  fprintf(fp, "      <rfc2543Hold>false</rfc2543Hold>\n");
  fprintf(fp, "      <callHoldRingback>2</callHoldRingback>\n");
  fprintf(fp, "      <localCfwdEnable>true</localCfwdEnable>\n");
  fprintf(fp, "      <semiAttendedTransfer>true</semiAttendedTransfer>\n");
  fprintf(fp, "      <anonymousCallBlock>2</anonymousCallBlock>\n");
  fprintf(fp, "      <callerIdBlocking>2</callerIdBlocking>\n");
  fprintf(fp, "      <dndControl>0</dndControl>\n");
  fprintf(fp, "      <remoteCcEnable>true</remoteCcEnable>\n");
  fprintf(fp, "    </sipCallFeatures>\n");
    ...
    if (strcmp(fields[i].xml, "lineType") == 0) {
      if (fields[i].opt_sel > 0) {
        if (fields[i].opt_sel == 1) { // Line
          char *ext = fields[i + 1].value;
          char *label = fields[i + 2].value[0] ? fields[i + 2].value : ext;
          char *auth = fields[i + 3].value[0] ? fields[i + 3].value : ext;
  ...
  fprintf(fp, "  </sipLines>\n");
  fprintf(fp, "  <voipControlPort>%s</voipControlPort>\n", port);
  char *rtpMin = val("startMediaPort");
  if (rtpMin[0]) {
    fprintf(fp, "  <startMediaPort>%s</startMediaPort>\n", rtpMin);
    fprintf(fp, "  <stopMediaPort>%s</stopMediaPort>\n", val("stopMediaPort"));
  }
  char *adscp = val("dscpForAudio");
  if (adscp[0])
    fprintf(fp, "  <dscpForAudio>%s</dscpForAudio>\n", adscp);
  fprintf(fp, "  <ringSettingBusyStationPolicy>0</ringSettingBusyStationPolicy>\n");
  fprintf(fp, "</sipProfile>\n");

  fprintf(fp, "  <commonProfile>\n");
  fprintf(fp, "    <phonePassword></phonePassword>\n");
  fprintf(fp, "  </commonProfile>\n");
...
void load_xml(const char *filename) {
  // Extract MAC from filename if possible
  Field *device_field = get_field("device");
  if (device_field) {
    const char *ptr = strstr(filename, "SEP");
    if (ptr) {
      char mac[13] = {0};
      strncpy(mac, ptr + 3, 12);
      strcpy(device_field->value, mac);
    }
  }
  ...
  // Read whole file into memory for easier multi-line parsing
  fseek(fp, 0, SEEK_END);
  long fsize = ftell(fp);
  fseek(fp, 0, SEEK_SET);
  ...
  for (int i = 0; i < total_fields; i++) {
    if (fields[i].xml && strlen(fields[i].xml) > 0) {
      char tag_open[128], tag_close[128];
      
      // Handle special pseudo-tags
      if (strcmp(fields[i].xml, "processNodeName1") == 0) {
        strcpy(tag_open, "<processNodeName>");
        strcpy(tag_close, "</processNodeName>");
      } else if (strcmp(fields[i].xml, "device") == 0) {
        continue; // MAC address is handled from filename, avoid parsing the root <device> tag
      } else if (strcmp(fields[i].xml, "userLocale") == 0) {
        char *ul_start = strstr(buffer, "<userLocale>");
        if (ul_start) {
          char *name_start = strstr(ul_start, "<name>");
          char *name_end = name_start ? strstr(name_start, "</name>") : NULL;
          char *ul_end = strstr(ul_start, "</userLocale>");
          if (name_start && name_end && ul_end && name_end < ul_end) {
            name_start += 6;
            int len = name_end - name_start;
            if (len >= sizeof(fields[i].value)) len = sizeof(fields[i].value) - 1;
            strncpy(fields[i].value, name_start, len);
            fields[i].value[len] = '\0';
            
            if (fields[i].opt_count > 0) {
              for (int j = 0; j < fields[i].opt_count; j++) {
                if (strcmp(fields[i].xml_values[j], fields[i].value) == 0) {
                  fields[i].opt_sel = j;
                  break;
                }
              }
        ...
        continue;
      } else if (strcmp(fields[i].xml, "ntpServer") == 0) {
        // NTP server is inside <ntp><name>
        char *ntp_start = strstr(buffer, "<ntp>");
        if (ntp_start) {
          char *name_start = strstr(ntp_start, "<name>");
        ...
        sprintf(tag_open, "<%s>", fields[i].xml);
        sprintf(tag_close, "</%s>", fields[i].xml);
      }

      char *start = strstr(buffer, tag_open);
      char *end = strstr(buffer, tag_close);
      
      // Prevent mapping line <name> tags to global device labels if they conflict
      if (strcmp(fields[i].xml, "name") == 0 && start) {
         // It's a line extension name. Find it inside <sipLines> instead of <userLocale><name>
         char *lines_start = strstr(buffer, "<sipLines>");
         if (lines_start) {
            start = strstr(lines_start, tag_open);
            end = start ? strstr(start, tag_close) : NULL;
         }
      }
      ...
      if (start && end && end > start) {
        ...
        int len = end - start;
        if (len >= sizeof(fields[i].value)) len = sizeof(fields[i].value) - 1;
        strncpy(fields[i].value, start, len);
        fields[i].value[len] = '\0';
        
        // Auto-select dropdown if matches
        if (fields[i].opt_count > 0) {
          for (int j = 0; j < fields[i].opt_count; j++) {
            if (strcmp(fields[i].xml_values[j], fields[i].value) == 0) {
              fields[i].opt_sel = j;
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    attron(COLOR_PAIR(2));
    mvprintw(0, 0,
             " CISCO 8945 CONFIG GENERATOR | Fields: %d | [s] Save | [q] Quit ",
             total_fields);
